(function app() {
    ...
    //   addCurrency();
      ...
      function getAndUpadteFormattedData (){
        let formattedName = myProductElement.querySelector(".c-product__name").innerText;
        dataJson.formattedName = formattedName;
        let myCurrency = myProductElement.querySelector("meta[itemprop='priceCurrency']").getAttribute('content');
        dataJson.currency = myCurrency;
        let formattedPrice = myProductElement.querySelector("p.c-price__value--current").innerText;
        dataJson.formattedPrice = formattedPrice;
      }
      return dataJson;
    }
    ...
    // Show the onscreen produts // Extend version, data already are formatted
    function showData(temporaryArray){
        let formattedDataArray = [];
        temporaryArray.map((temporaryItem) => {
            let formattedData = {
              id: temporaryItem?.id,
              name: temporaryItem?.formattedName,
              price: temporaryItem?.formattedPrice
            };
